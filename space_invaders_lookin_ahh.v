         ...
         /bullet[2:0]
            ...
            $hit_enemy[1:0] = ($reset || !>>1$bullet_exists) ? 2'b0 :
                              ...
                              (>>1$bullet_dir[0] == 1'b1) ?
                                 ...
                                 ((/player/other_player/ship[1]>>1$xx_p + 8'b10000000) < (- >>1$bullet_x + 8'b10000000 + 8'd6)) &&
                                 ((/player/other_player/ship[1]>>1$yy_p + 8'b10000000) > (- >>1$bullet_y + 8'b10000000 - 8'd13)) &&
                                 ((/player/other_player/ship[1]>>1$yy_p + 8'b10000000) < (- >>1$bullet_y + 8'b10000000 + 8'd13))) :
                              (((/player/other_player/ship[1]>>1$xx_p + 8'b10000000) > (- >>1$bullet_x + 8'b10000000 - 8'd13)) &&
                              ((/player/other_player/ship[1]>>1$xx_p + 8'b10000000) < (- >>1$bullet_x + 8'b10000000 + 8'd13)) &&
                              ((/player/other_player/ship[1]>>1$yy_p + 8'b10000000) > (- >>1$bullet_y + 8'b10000000 - 8'd6)) &&
                              ((/player/other_player/ship[1]>>1$yy_p + 8'b10000000) < (- >>1$bullet_y + 8'b10000000 + 8'd6))),
                              /player/other_player/ship[0]>>1$destroyed ? 1'b0 :
                              (>>1$bullet_dir[0] == 1'b1) ?
                                 (((/player/other_player/ship[0]>>1$xx_p + 8'b10000000) > (- >>1$bullet_x + 8'b10000000 - 8'd6)) &&
                                 ((/player/other_player/ship[0]>>1$xx_p + 8'b10000000) < (- >>1$bullet_x + 8'b10000000 + 8'd6)) &&
                                 ((/player/other_player/ship[0]>>1$yy_p + 8'b10000000) > (- >>1$bullet_y + 8'b10000000 - 8'd13)) &&
                                 ((/player/other_player/ship[0]>>1$yy_p + 8'b10000000) < (- >>1$bullet_y + 8'b10000000 + 8'd13))) :
         ...
         \viz_js
            ...
            render() {
               ...
               if (this.last_cycle <= this.getCycle()) // -------- If Moving Forward Cycles --------
               {
                  ...
                  this.getObjects().shield_meter.set({
                     left: current_ship_img.left + shield_meter_x_offset,
                     top: current_ship_img.top + shield_meter_y_offset,
                  ...
                  if ('$destroyed'.asBool() && !'>>1$destroyed'.asBool())
                  {
                     for (let i = 0; i < 5; i++)
                     {
                        this.getObjects()[`explody_sprite${i}`]
                           .set({visible: false})
                           .wait(i * 140)
                  ...
                  this.getObjects().shield_meter_back.animate({
                     ...
                     duration: 180,
                     onComplete: () => {this.getObjects().shield_meter_back.set({ visible: !'$destroyed'.asBool()})},
                     easing: fabric.util.ease.easeOutCubic
                  });
                  this.getObjects().shield_meter.animate({
                     left: current_xx_p + shield_meter_x_offset,
                     top: current_yy_p + shield_meter_y_offset,
                     scaleX: //If in shield_up phase:
                  ...
                  this.getObjects().shield_img.animate({
                     ...
                     scaleY: '$shield_up'.asBool() ? 1.0 :
                             '$hit'.asBool() ? 2.0 : 0.0,
                     opacity: '$hit'.asBool() ? 0.0 : 1.0
                  }, {
                     duration: 180,
                     onComplete: () => {this.getObjects().shield_img.set({ visible: !'$destroyed'.asBool() && this.getObjects().shield_img.visible })},
                     easing: fabric.util.ease.easeOutCubic
                  });
               }
               else // -------- If Moving Backward Cycles --------
               {
                  ...
                  if ('$destroyed'.step().asBool() && !'$destroyed'.asBool())
                  {
                     for (let i = 0; i < 5; i++)
                     {
                        this.getObjects()[`explody_sprite${4 - i}`]
                           .set({visible: false})
                           .wait(i * 140)
                           .thenSet({left: current_xx_p, top: current_yy_p, visible: true})
                           .thenWait(140).thenSet({visible: false});
                  ...
                  this.getObjects().shield_img.animate({
                     ...
                     duration: 180,
                     onComplete: () => {this.getObjects().shield_img.set({ visible: '$shield_up'.asBool()})},
                     easing: fabric.util.ease.easeOutCubic
                  });
               }
            this.last_cycle = this.getCycle();
      ...
      \viz_js
         ...
         render()
         {
            ...
            if (this.last_cycle < this.getCycle()) // ------- If Moving Forward Cycles -------
            {
               ...
               {
                  this.getObjects().p1win_img.set({visible: false});
                  this.getObjects().p2win_img.set({visible: false});
